...
(()=>{
  ...
  async function ensureV2(){if(window.FTNFileV2)return;await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='/ftn-file-v2.js';s.onload=resolve;s.onerror=()=>reject(Error('Encrypted file engine unavailable'));document.head.appendChild(s)})}
  async function encryptAttachment(file,pass,onProgress){await ensureV2();return FTNFileV2.encrypt(file,pass,onProgress)}
  async function decryptAttachment(blob,pass,onProgress){const pkg=JSON.parse(await blob.text());if(pkg.format!=='FTN-FILE-AES-GCM')throw Error('Unsupported FTN attachment format');if(pkg.version===2){await ensureV2();return FTNFileV2.decrypt(blob,pass,onProgress)}if(pkg.version!==1)throw Error('Unsupported FTN attachment version');const k=await key(pass,unb64(pkg.salt));const data=await crypto.subtle.decrypt({name:'AES-GCM',iv:unb64(pkg.iv)},k,unb64(pkg.data));if(Number(pkg.size)>=0&&Number(pkg.size)!==data.byteLength)throw Error('Attachment integrity check failed');return {blob:new Blob([data],{type:pkg.type||'application/octet-stream'}),name:pkg.name||'decrypted-file',type:pkg.type||'application/octet-stream',size:Number(pkg.size)||data.byteLength}}
  ...
  async function fetchAttachment(id,filename,pass,onProgress){if(!token())throw Error('Sign in required');const ft=await fileToken();const url=PB+'/api/files/notes/'+encodeURIComponent(id)+'/'+encodeURIComponent(filename)+'?token='+encodeURIComponent(ft);const r=await fetch(url);if(!r.ok)throw Error('Encrypted attachment download failed');return decryptAttachment(await r.blob(),pass,onProgress)}
  function safeAttachmentName(){return 'attachment.ftnenc'}
  async function save(note,{title,body,pass,folder='',tags='',favorite=false,archived=false,attachment=null,removeAttachment=false,onProgress}){
    ...
    const form=new FormData();Object.entries(payload).forEach(([k,v])=>form.append(k,String(v)));form.append('attachment',await encryptAttachment(attachment,pass,onProgress),safeAttachmentName());
  ...
  window.FTNNoteEditor={seal,openBox,encryptAttachment,decryptAttachment,fetchAttachment,save,load,remove};
})();
