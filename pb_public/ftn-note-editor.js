...
(()=>{
  ...
  async function decryptAttachment(blob,pass){const pkg=JSON.parse(await blob.text());if(pkg.format!=='FTN-FILE-AES-GCM'||pkg.version!==1)throw Error('Unsupported FTN attachment format');const k=await key(pass,unb64(pkg.salt));const data=await crypto.subtle.decrypt({name:'AES-GCM',iv:unb64(pkg.iv)},k,unb64(pkg.data));return {blob:new Blob([data],{type:pkg.type||'application/octet-stream'}),name:pkg.name||'decrypted-file',type:pkg.type||'application/octet-stream',size:Number(pkg.size)||data.byteLength}}
