...
(()=>{
  ...
  async function encryptAttachment(file,pass,onProgress){if(!window.FTNFileV2)throw Error('Encrypted file engine unavailable');return FTNFileV2.encrypt(file,pass,onProgress)}
  async function decryptAttachment(blob,pass,onProgress){const pkg=JSON.parse(await blob.text());if(pkg.format!=='FTN-FILE-AES-GCM')throw Error('Unsupported FTN attachment format');if(pkg.version===2){return FTNFileV2.decrypt(blob,pass,onProgress)}if(pkg.version!==1)throw Error('Unsupported FTN attachment version');const k=await key(pass,unb64(pkg.salt));const data=await crypto.subtle.decrypt({name:'AES-GCM',iv:unb64(pkg.iv)},k,unb64(pkg.data));if(Number(pkg.size)>=0&&Number(pkg.size)!==data.byteLength)throw Error('Attachment integrity check failed');return {blob:new Blob([data],{type:pkg.type||'application/octet-stream'}),name:pkg.name||'decrypted-file',type:pkg.type||'application/octet-stream',size:Number(pkg.size)||data.byteLength}}
