...
(()=>{
  ...
  async function createFolder(name,parent=''){name=String(name||'').trim();if(!name)throw Error('Folder name required');const clean=name.replace(/[\\/]+/g,' ').replace(/\s+/g,' ').trim();if(!clean)throw Error('Invalid folder name');const path=(parent?parent+'/':'')+clean;return api('/api/collections/file_folders/records',{method:'POST',body:JSON.stringify({user:user().id,name:clean,parent,path})})}
