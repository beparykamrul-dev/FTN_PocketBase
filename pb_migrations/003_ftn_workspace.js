migrate((app) => {
  ...
  const folders = new Collection({
    ...
    fields: [new RelationField({name:'user',collectionId:users.id,required:true,maxSelect:1}), new TextField({name:'name',required:true,max:120}), new RelationField({name:'parent',collectionId:'file_folders',maxSelect:1}), new TextField({name:'path',required:true,max:500})]
  });
  app.save(folders);
