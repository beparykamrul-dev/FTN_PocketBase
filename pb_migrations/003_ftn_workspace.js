migrate((app) => {
  ...
  const folders = new Collection({
    ...
    fields: [new TextField({name:'user',required:true}), new TextField({name:'name',required:true,max:120}), new TextField({name:'parent',max:15}), new TextField({name:'path',required:true,max:500})]
  });
  app.save(folders);
  ...
  const c = app.findCollectionByNameOrId('file_folders'); if (c) app.delete(c);
});
